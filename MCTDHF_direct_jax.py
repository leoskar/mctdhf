...
class MCTDHF:
    ...
    def __init__(self, h, ht, g, num_mctdhf_orbitals, num_spatial_orbitals, 
        ...
        self.C_shape = (self.num_alpha_dets, self.num_beta_dets)
    ...
    def get_E(self, t, b, C):
        ...
        return self.dCI.calculate_energy(jnp.reshape(C, self.C_shape), h_2, g_4)
    ...
    def __call__(self, t, y):
        ...
        b_dot = -self.time*(h_1 - h_3 + contract('np, pqrs, iqrs -> in', D_inv, d, g_3-g_5, backend='jax'))
    ...
    def integrate(self, num_steps: int, dt: float, t_init = 0, b_init = None, C_init = None, normalize = None):
        ...
        if C_init is None:
            # Random initial C
            rng = np.random.default_rng()
            C_init = jnp.array(rng.random(self.num_slater_dets), jnp.complex64)
            C_init = normalize_C(C_init)
    ...
    def calculate_overlap(self, b1, C1, b2, C2):
        ...
        if self.num_beta_electrons == self.num_alpha_electrons:
            return contract('ik,jl,ij,kl', C1.reshape(self.C_shape).conj(), C2.reshape(self.C_shape), S_a, S_a, backend='jax')
        ...
        return contract('ik,jl,ij,kl', C1.reshape(self.C_shape).conj(), C2.reshape(self.C_shape), S_a, S_b, backend='jax')
    

    def calculate_particle_density(self, b, C, spf):
        """Calculate the particle density"""

        D = self.dCI.get_1p_RDM(C.reshape(self.C_shape))
