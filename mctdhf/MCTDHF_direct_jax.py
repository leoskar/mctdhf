...
class MCTDHF:
    ...
    def __init__(
        ...
        self.dCI = SemiDirectCI(
            self.num_mctdhf_orbitals, num_alpha_electrons, num_beta_electrons
        )
    ...
    def ortonorm_b(self, b):
        ...
        for i in range(b.shape[1]):
            orto_adjustment = 0
            for j in range(0, i):
                orto_adjustment += b[:, j] * contract(
                    "i,j,ij", b[:, i], b[:, j], self.S, backend="jax"
                )
    ...
    def __call__(self, t, y):
        ...
        b_dot = -self.time * (
            Sh_1
            - h_3
            + contract("np, pqrs, iqrs -> in", D_inv, d[0], Sg_3 - g_5, backend="jax")
        )
    ...
    def calculate_overlap(self, b1, C1, b2, C2):
        ...
        I_alpha = SemiDirectCI.get_slater_dets(
            self.num_mctdhf_orbitals, self.num_alpha_electrons
        )

        s_a = contract("ai,ki,kj,bj->abij", I_alpha, b1, b2, I_alpha, backend="jax")
        s_a = s_a[
            jnp.nonzero(s_a, size=(self.num_alpha_dets * self.num_alpha_electrons) ** 2)
        ]
        S_a = jnp.linalg.det(
            s_a.reshape((self.num_alpha_dets,) * 2 + (self.num_alpha_electrons,) * 2)
        )

        if self.num_beta_electrons == self.num_alpha_electrons:
            return contract("cik,cjl,ij,kl->c", C1.conj(), C2, S_a, S_a, backend="jax")

        I_beta = SemiDirectCI.get_slater_dets(
            self.num_mctdhf_orbitals, self.num_beta_electrons
        )

        s_b = contract("ai,ki,kj,bj->abij", I_beta, b1, b2, I_beta, backend="jax")
        s_b = s_b[
            jnp.nonzero(s_b, size=(self.num_beta_dets * self.num_beta_electrons) ** 2)
        ]
        S_b = jnp.linalg.det(
            s_b.reshape((self.num_beta_dets,) * 2 + (self.num_beta_electrons,) * 2)
        )
    ...
    def plot_Cbe(self, ts, Cs, bs, Es):
        ...
        axs[1].plot(
            ts,
            np.real(
                bs.reshape(-1, self.num_spatial_orbitals, self.num_mctdhf_orbitals)[
                    :, :, 0
                ]
            ),
        )
    ...
    def get_full_basis_1p_RDM(self, Cs, bs):
        Ds = self.dCI.get_1p_RDM(Cs)
        return contract("tin, tnm, tjm -> tij", bs, Ds, bs, backend="jax")

    def expectation_val(self, op, Cs, bs):
        rho = self.get_full_basis_1p_RDM(Cs, bs)
        return contract("pq, tqp -> t", op, rho)
